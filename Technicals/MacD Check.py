...
def compare_with_yahoo_finance(ticker):
    yahoo_data = yf.download(ticker, start="2020-01-01", end="2024-01-01")
    yahoo_macd = yahoo_data['Close'].ewm(span=12, min_periods=0, adjust=False).mean() - yahoo_data['Close'].ewm(span=26, min_periods=0, adjust=False).mean()
    last_yahoo_macd = yahoo_macd.iloc[-1]
    print(f"Yahoo Finance MACD: {last_yahoo_macd}")

def main():
    tickers = ['BJRI', 'MSFT', 'GOOGL', 'AMZN']
    data_list = []
    ...
    for ticker in tickers:
        ...
        if result:
            ...
            compare_with_yahoo_finance(ticker)
        else:
    ...
    excel_file = os.path.join(export_path, f"MACD_200D_signals_{timestamp}.xlsx")
